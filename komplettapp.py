    ...
    if st.button("Logga in"):
        if os.path.exists("användarprofiler.csv"):
            ...
            if not match.empty:
                ...
                # Ladda ALLA forum-inlägg
                if os.path.exists("forum.csv"):
                    df_forum = pd.read_csv("forum.csv", on_bad_lines='skip')
                    st.session_state.forum_posts = df_forum.to_dict(orient="records")
                else:
                    st.session_state.forum_posts = []
                # Ladda ALLA rapporter
                if os.path.exists("rapporter.csv"):
                    df_reports = pd.read_csv("rapporter.csv", on_bad_lines='skip')
                    st.session_state.reports = df_reports.to_dict(orient="records")
                else:
                    st.session_state.reports = []
                # Ladda SPARADE RUTTER (bara användarens)
...
# ----------------- RUTTER -----------------
    ...
    start_address = st.text_input("Startadress", value=st.session_state.get("start_address_prefill", ""))
    end_address = st.text_input("Slutadress", value=st.session_state.get("end_address_prefill", ""))
...
# ----------------- SPARADE RUTTER -----------------
    ...
    if st.session_state.saved_routes:
        for i, r in enumerate(st.session_state.saved_routes):
            ...
            with col2:
                if st.button("🧭 Navigera", key=f"navigate_{i}"):
                    # Sätt in rutten i session_state och hoppa till Utforska rutter
                    st.session_state.start_address_prefill = r['start']
                    st.session_state.end_address_prefill = r['end']
                    st.session_state.page = "rutter"
                    st.rerun()
            with col3:
...
# ----------------- FORUM -----------------
    ...
    if st.session_state.forum_posts:
        for i, post in enumerate(reversed(st.session_state.forum_posts)):
            actual_index = len(st.session_state.forum_posts) - 1 - i
            ...
            with col2:
                if post['email'] == user_info["email"]:
                    if st.button("🗑️ Ta bort", key=f"delete_forum_{i}"):
                        st.session_state.forum_posts.pop(actual_index)
                        if os.path.exists("forum.csv"):
                            df = pd.read_csv("forum.csv", on_bad_lines='skip')
                            df = df[~(
                                (df['email'] == post['email']) &
                                (df['name'] == post['name']) &
...
# ----------------- RAPPORTERING -----------------
    ...
    if st.session_state.reports:
        for i, r in enumerate(reversed(st.session_state.reports)):
            actual_index = len(st.session_state.reports) - 1 - i
            col1, col2 = st.columns([5, 1])
            with col1:
                ...
                st.markdown("---")
            with col2:
                if r['email'] == user_info["email"]:
                    if st.button("🗑️ Ta bort", key=f"delete_report_{i}"):
                        st.session_state.reports.pop(actual_index)
                        if os.path.exists("rapporter.csv"):
                            df = pd.read_csv("rapporter.csv", on_bad_lines='skip')
                            df = df[~(
                                (df['email'] == r['email']) &
                                (df['address'] == r['address']) &
                                (df['type'] == r['type']) &
                                (df['description'] == r['description'])
                            )]
                            df.to_csv("rapporter.csv", index=False)
                        st.success("✅ Rapport borttagen!")
                        st.rerun()
    else:
